...
#1. 데이터
train = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/ai_sub/dacon/computer/train.csv')
test = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/ai_sub/dacon/computer/test.csv')
...
train = train.drop(['id', 'digit', 'letter'], axis=1).values
...
for i in range(5): 
  for j in range(10): 
    axs[i, j].imshow(tf.squeeze(test[a])) 
...
#노이즈 제거
noise_factor = 0.4
x_train_noisy = x_train + noise_factor*tf.random.normal(shape = x_traun.shape)
#범위 벗어난 값 자르기
...
lr1 = ReduceLROnPlateau( monitor='val_loss', factor=0.3, patience=6, verbose=1, mode='auto')
skf = StratifiedKFold(n_splits= 32, random_state=66, shuffle=True)
...
for train_index, valid_index in skf.split(data,train['digit']) :
    ...
    train_generator = idg.flow(x_train,y_train, batch_size = 12)
    ...
    conv2d = Conv2D(64, (4), strides =1 ,padding = 'SAME', input_shape = (28,28,1))(inputs)
    btcn = BatchNormalization()(conv2d)
    conv2d = Conv2D(128, (3), strides =1, padding = 'SAME', activation='relu')(btcn)
    btcn = BatchNormalization()(conv2d)
    conv2d = Conv2D(256, (3), strides =1, padding = 'SAME', activation='relu')(btcn)
    ...
    dense = Dense(256, activation='relu')(btcn)
    btcn = BatchNormalization()(dense)
    drop = Dropout(0.2)(btcn)
...
#3. 훈련
    ...
    learning_hist = model.fit_generator(train_generator, epochs = 200, validation_data=(valid_generator), verbose = 1 ,callbacks = [es, lr1]) #mc
    ...
    result += model.predict_generator(test_generator,verbose=True)/32
...
#4. 평가, 예측
submission = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/ai_sub/dacon/computer/submission.csv')
submission['digit'] = result.argmax(1)
submission.to_csv('/content/drive/MyDrive/Colab Notebooks/ai_sub/dacon/computer/2021.02.05_1.csv',index=False)
