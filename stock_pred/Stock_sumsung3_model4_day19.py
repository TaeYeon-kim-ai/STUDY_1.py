...
lstm1 = LSTM(230,activation='relu',input_shape = (x1.shape[1], x1.shape[2]))(input1)
drop1 = Dropout(0.1)(lstm1)
dense1 = Dense(100, activation='relu')(drop1)
dense1 = Dense(100, activation='relu')(drop1)
dense1 = Dense(72, activation='relu')(drop1)
...
lstm2 = LSTM(230,activation='relu',input_shape = (x2.shape[1], x2.shape[2]))(input2)
...
dense2 = Dense(72, activation='relu')(dense2)
...
middle = Dense(100, activation= 'relu')(merge)
...
hist = model.fit([x1_train, x2_train], y_train, epochs = 1000, batch_size = 32, validation_data = ([x1_val, x2_val], y_val), verbose = 1 ,callbacks = [early_stopping]) #cp])
...
#4. 평가, 예측
loss = model.evaluate([x1_test, x2_test], y_test ,batch_size=7)
print('loss : ', loss)
...
# loss :  [388578.15625, 496.16986083984375]
# 주가 : [[84846.07 84860.38]]

# loss :  [6901673.5, 2509.802734375]
# 주가 : [[85516.2   85491.625]]

# loss :  [816738.3125, 763.4340209960938]
# 주가 : [[84750.69 84777.76]]