...
class EfficientNet_MultiLabel(nn.Module):
    def __init__(self, in_channels):
        ...
        self.network = EfficientNet.from_pretrained('efficientnet-b5', in_channels=in_channels)
...
kf = KFold(n_splits=5, shuffle=True, random_state=42) #초기값 5
folds=[]
...
#     model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/data/torch_model/EfficientNetB5-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/data/torch_model/EfficientNetB5-fold{}.pt'.format(fold)))
        model.eval()
