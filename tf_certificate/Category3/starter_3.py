...
def solution_model():
    ...
    validation_generator = validation_datagen.flow_from_directory('tmp/testdata',
                                 batch_size=16, class_mode='binary', target_size=(300, 300))
        #Your Code Here)
    ...
    model.compile(loss = 'binary_crossentropy', optimizer = RMSprop(lr=0.001), metrics=['acc'])#Your Code Here#)

    model.fit(train_generator, steps_per_epoch=8, epochs=20, verbose=1,
                validation_data=validation_generator, validation_steps=8)#Your Code Here#)
