...
x = [] #x에 첨부
for i in range(1,26): # i  1~2까지 소스 가져오기
    df = pd.read_csv(f'C:/data/vision_2/submission_2021.02.13_{i}.csv', index_col=0, header=0) # 데이터 프레임 가져오기
    data = df.to_numpy() # df를  numpy로 전환
    x.append(data) 

x = np.array(x)

df = pd.read_csv(f'C:/data/vision_2/submission_2021.02.13_{i}.csv', index_col=0, header=0)
for i in range(5000):
    for j in range(26):
        a = []
        for k in range(1):
...
y.to_csv('C:/data/vision_2/submission_2021.02.14.csv')  
